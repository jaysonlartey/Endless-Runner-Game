...
## Purpose and Context
This project was created as part of the **Game Programming** course to apply game development concepts in building an interactive game. The idea is to explore **game mechanics**, **UI/UX design**, and **game state management** while implementing features such as **procedural level generation** and **progressive difficulty**. The game is inspired by popular games like **Geometry Dash**, **Subway Surfers**, and **Extreme Pamplona**.

## Key Features
- **Keyboard Controls**: Players can control the character using keyboard inputs (`SPACE` for jumping and `S` for sliding).
- **Procedural Level Generation**: The environment updates at each level change, including background variations to keep levels visually distinct.
